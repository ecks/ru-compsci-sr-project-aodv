										 ...
										 //double expiry_time - not sure if we need it???
{
	...
	if (hash_key_addr_family == InetC_Addr_Family_v4)
		geo_table_ptr->geo_table = inet_addr_hash_table_create (GEOTABLE_SIZE, 0);
	else
		geo_table_ptr->geo_table = inet_addr_hash_table_create (0, GEOTABLE_SIZE);
...
// Deteremine if the GeoTable contains the entry with the given IP address
...
// Insert a new entry into GeoTable
...
// In:		geo_table_ptr -- pointer to the GeoTable
...
//			address -- IP address of the node to be added into the GeoTable
//
// Out: 	OPC_COMPCODE_SUCCESS if entry was removed
//			OPC_COMPCODE_FAILURE if entry does not exist
//
// Uses:	aodv_geo_table_entry_exists
//			aodv_geo_table_entry_delete
...
// Purpose:  retrieve an entry from the GeoTable
